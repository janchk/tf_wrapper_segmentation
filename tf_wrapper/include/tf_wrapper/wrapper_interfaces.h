...
public:
  bool setGpuNumberPreferred(int value) override {
    return segm.setGpuNumberPreferred(value);
  }
  ...
  bool setSegmentationColors(std::vector<std::array<int, 3>> colors) override {
    return segm.setSegmentationColors(colors);
  }

  bool set_input_output(std::vector<std::string> in_nodes,
                        std::vector<std::string> out_nodes) override {
    return segm.setInputOutput(in_nodes, out_nodes);
  }
  ...
  std::string getVisibleDevices() override {
    return segm.getVisibleDevices();
  } // Does not really work yet

  std::vector<cv::Mat> getOutputSegmentationIndices() override {
    return segm.getOutputSegmentationIndices();
  }

  std::vector<cv::Mat> getOutputSegmentationColored() override {
    return segm.getOutputSegmentationColored();
  }
...
public:
  bool set_config_path(std::string path) override {
    return db.set_config_path(path);
  };
  ...
  cv::Size get_config_input_size() override {
    return db.get_config_input_size();
  }

  std::string get_config_input_node() override {
    return db.get_config_input_node();
  }

  std::string get_config_output_node() override {
    return db.get_config_output_node();
  }

  std::string get_config_pb_path() override { return db.get_config_pb_path(); }

  std::string get_config_colors_path() override {
    return db.get_config_colors_path();
  }
