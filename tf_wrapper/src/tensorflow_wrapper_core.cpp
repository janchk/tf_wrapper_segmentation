...
void TensorflowWrapperCore::clearSession() { _output_tensors.clear(); }
...
void TensorflowWrapperCore::configureGraph() {
  ...
  if (!_cpu_only && _gpu_number >= 0)
    graph::SetDefaultDevice("/job:localhost/replica:0/task:0/device:GPU:" +
                                std::to_string(_gpu_number),
                            &_graph_def);
  //        graph::SetDefaultDevice("/device:GPU:" +
...
std::string TensorflowWrapperCore::inference(const std::vector<cv::Mat> &imgs) {
  ...
  //    Tensor input = getConvertFunction(INPUT_TYPE::DT_FLOAT)(imgs,
  //    _input_height, _input_width, _input_depth, _convert_to_float, _mean);
  Tensor input;
...
void TensorflowWrapperCore::setName(const std::string &name) { _name = name; }
...
void TensorflowWrapperCore::getInputNodeNameFromGraphIfPossible(
    const std::string &inputNodeName) {
  using namespace tensorflow;
...
tensorflow::Tensor
TensorflowWrapperCore::getTensorFromGraph(const std::string &tensor_name) {
  using namespace tensorflow;
...
bool TensorflowWrapperCore::getAllowGrowth() const { return _allow_growth; }
...
void TensorflowWrapperCore::setVisibleDevices(
...
int TensorflowWrapperCore::getGpuNumber() const { return _gpu_number; }

void TensorflowWrapperCore::setGpuNumber(int value) { _gpu_number = value; }
...
bool TensorflowWrapperCore::getCpuOnly() const { return _cpu_only; }

void TensorflowWrapperCore::setCpuOnly(bool cpu_only) { _cpu_only = cpu_only; }
...
std::string TensorflowWrapperCore::getPath() const { return _path; }
