...
fs_img::image_data_struct fs_img::read_img(const std::string &im_filename,
                                           cv::Size &size) {
  ...
  tf_aux::fastResizeIfPossible(out_data.img_data,
                               const_cast<cv::Mat *>(&out_data.img_data), size);
...
std::vector<std::string> fs_img::list_imgs(const std::string &dir_path) {
  ...
  for (const auto &entry : fs::recursive_directory_iterator(dir_path)) {
    if (fs::is_regular_file(entry) &&
        (fs::path(entry.path()).extension() == "JPG" || "jpg"))
