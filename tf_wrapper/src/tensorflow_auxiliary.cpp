...
bool tf_aux::convertMatToTensor_v2(const std::vector<cv::Mat> &imgs,
...
bool tf_aux::fastResizeIfPossible(const cv::Mat &in, cv::Mat *dist,
                                  const cv::Size &size) {
  if (in.size() == size) {
    DebugOutput("sizes matches", std::to_string(in.cols) + "x" +
